  ...
  return (
    <div className='mt-16 pb-16'>
        <div className='flex flex-col items-end mb-8 w-max'>
            <p className='font-semibold text-2xl uppercase'>My Orders</p>
            ...
            <div key={index} className='border border-gray-300 rounded-lg mb-10 p-4 py-5 max-w-4xl '>
                ...
                {order.items.map((item, index)=>(
                    <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index+1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl ` }>
                        <div className='mg:mb-0 mb-4 flex items-center'>
                            <div className='bg-primary/10 p-4 rounded-lg'>
                                <img src={item.product.image[0]} alt="" className='w-16 h-16' />
                            </div>
                            <div className='ml-4'>
                                <h2 className='text-gray-800 font-medium text-xl'>{item.product.name}</h2>
